...
def get_bool_object_type(obj) -> int:
    if obj:
        return 1
    return 2


def get_set_object_type(obj) -> int:
    return 3


def get_datetime_object_time(obj) -> int:
    return 6


def get_int_object_type(obj) -> int:
    if obj < 0:
        return get_float_object_type(obj)
    elif obj <= 0x27:
        return obj + 8
...
def get_float_object_type(obj) -> int:
...
def get_str_object_type(obj) -> int:
...
def get_bytes_object_type(obj) -> int:
    obj_len = len(obj)
    if obj_len <= 0x20:
        return 0x70 + obj_len
    elif obj_len <= 0xFF:
...
def get_list_object_type(obj) -> int:
    if len(obj) < 15:
        return 0xD0 + len(obj)
    else:
        obj = copy.copy(obj)
...
def get_dict_object_type(obj) -> tuple[int, list]:
    obj_len = len(obj)
    obj = list(obj.items())
    if obj_len < 15:
...
OBJ_TYPE_MAP = {
    bool: get_bool_object_type, datetime.datetime: get_datetime_object_time, TerminatorObject: get_set_object_type,
    int: get_int_object_type, float: get_float_object_type, str: get_str_object_type,
    bytes: get_bytes_object_type, list: get_list_object_type, dict: get_dict_object_type
}
